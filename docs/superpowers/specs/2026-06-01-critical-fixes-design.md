...
**Date**: 2026-06-01
**Scope**: nexad, nexa-cli, install.sh, CI workflows
...
8 CRITICAL issues remain after nexa-proxy removal. Grouped into 3 blocs:

| Bloc | Issues | Crates |
|------|--------|--------|
| 1 — Security | API auth, bind address, join token hash, secrets stdin | nexad, nexa-cli |
| 2 — Correctness | stop/remove pod, graceful shutdown | nexad |
...
- **Override**: `nexad --api-token <token>` sets a specific token (hashed before storage). Useful for CI/automation.
...
- **Files**: `nexad/src/api/auth.rs` (new), `nexad/src/api/routes.rs`, `nexad/src/api/mod.rs`, `nexad/src/main.rs`
...
- **Files**: `nexad/src/main.rs` (2 lines)
...
- **Files**: `nexad/src/api/handlers.rs`

### CRITICAL 4: Read secrets from stdin instead of CLI arguments

**Problem**: `nexa secret set DB_PASS s3cret -p myapp` exposes the secret in `ps aux`, shell history, and audit logs.
...
- Usage: `echo $SECRET | nexa secret set DB_PASS -p myapp` or `nexa secret set DB_PASS -p myapp` (interactive prompt).
...
- **Files**: `nexa-cli/src/main.rs`, `nexa-cli/src/commands/secret.rs`
...
- **Files**: `nexad/src/cluster/server.rs`
...
- **Files**: `nexad/src/main.rs` (all 3 modes: single, master, worker)
...
- **Files**: `install.sh`, `.github/workflows/release.yml` (in nexad repo)
...
- **Files**: `.github/workflows/release.yml` (in nexad repo), `install.sh`
...
4. CRITICAL 4 (secrets stdin) — nexa-cli change, independent
